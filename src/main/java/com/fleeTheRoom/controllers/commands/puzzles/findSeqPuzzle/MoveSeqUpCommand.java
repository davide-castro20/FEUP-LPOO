...
public class MoveSeqUpCommand implements Command {
    private final FindSeqPuzzle puzzle;

    public MoveSeqUpCommand(FindSeqPuzzle findSeqPuzzle) { this.puzzle = findSeqPuzzle; }

    @Override
    public void execute() {
        if(puzzle.getState() == FindSeqPuzzle.STATE.FINDING)
            puzzle.moveSelectionTo(puzzle.getCurrentSequence().up());
    }
}
